...
# Здесь пишем код
class Segment: # Объявляем класс
    def __init__(self, point1, point2): # создаем конструктор и передаем параметр Self
        self.point1 = point1 # присваиваем атрибут экземпляру объекта
        self.point2 = point2

    def length(self): # определяем метод, который вычисляет длину отрезка между двумя точками
        x1, y1 = self.point1
        x2, y2 = self.point2
        return round(((x2 - x1) ** 2 + (y2 - y1) ** 2) ** 0.5, 2)

    def x_axis_intersection(self):# определяем метод, которыйпроверяет, пересекает ли отрезок ось Х
        x1, y1 = self.point1
        x2, y2 = self.point2
        if y1 == 0 and y2 == 0:
            return False
    ...
    def y_axis_intersection(self):# определяем метод, которыйпроверяет, пересекает ли отрезок ось Y
        x1, y1 = self.point1
        x2, y2 = self.point2
        if x1 == 0 and x2 == 0:
            return False
