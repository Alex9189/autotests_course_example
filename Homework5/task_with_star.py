...
def to_roman(val):
    ...
    # Создаем словарь, где ключами являются арабские числа, а значениями - соответствующие им римские цифры
    roman_numeral_map = {
        1000: 'M',
        900: 'CM',
        500: 'D',
    ...
    roman_str = ''
    for n in roman_numeral_map: # итерируемся по всем значениям словаря
        while val >= n: # проверяем, больше ли или равно переданное число текущему ключу
            roman_str += roman_numeral_map[n] # к строке roman_str добавляется значение из словаря
            val -= n # из переданного числа вычитаем значение ключа n пока переданное число не станет меньше текущего ключа
    return roman_str
