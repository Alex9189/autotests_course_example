...
def repeats(our_str):
    ...
    new_str = ""
    for i in our_str:
        if i in count_dict: # если символ уже встречался
            count_dict[i] += 1 # то ее значение увеличивается на 1
        else: # если символ еще не встречался
            count_dict[i] = 1 # то добавляется со значением 1
        new_str += i + "_" + str(count_dict[i]) # составляем строеку из симвовлов и добавляем постфикс
    return new_str
